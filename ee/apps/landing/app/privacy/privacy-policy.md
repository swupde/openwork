...
If you choose to connect Google Workspace to OpenWork, OpenWork requests only the Google OAuth scopes needed for the capabilities you enable. Depending on your connection's permissions, these capabilities can include reading calendar events and Gmail messages, creating drafts for review, sending existing drafts, managing messages and labels, and reading or managing Drive files, documents, and spreadsheets. Additional management permissions are off by default; existing connections are not silently granted broader access.

OpenWork uses Google Workspace data only to provide the user-facing features you request, such as preparing for a meeting, reading an explicitly selected Drive file, or creating a Gmail draft. OpenWork does not sell Google user data, use Google user data for advertising, or use Google user data to train generalized AI models. Creating a Gmail draft does not send email. Sending an existing draft is a separate operation that requires the appropriate Google permissions and your authorization; permission to create or review a draft is not permission to send it.
