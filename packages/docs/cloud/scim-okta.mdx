...
SCIM can adopt an existing OpenWork account only when that account already has
an active membership in the connector's organization. Adoption preserves the
existing user ID, organization membership, and role, and places the identity
under SCIM management for subsequent updates and deactivation. New-user
provisioning is unchanged.

An email or domain match alone is not enough. Removed memberships, pending
invitations, personal accounts without an active membership in this organization,
and membership only in another organization cannot be adopted; creation returns
`409` with `scimType: uniqueness`. A token without an organization cannot adopt
an existing account either. Complete the normal organization joining process
before provisioning an existing account; do not use SCIM to bypass it.
...
| Okta reports `409 User already exists` | The existing account lacks an active membership in this connector's organization, or the provider account already exists | Check membership in the selected organization and inspect the System Log. Removed or invited-only members cannot be adopted. If a successful create immediately precedes the conflict, stop triggering **Provision User**. Use a never-before-provisioned user for the clean lifecycle test |
