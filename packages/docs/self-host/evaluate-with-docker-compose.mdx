...
| `OPENWORK_OWNER_EMAILS` | empty | Comma-separated emails allowed to claim the organization at `/setup`. Empty disables the private first-administrator flow. |
| `OPENWORK_SETUP_CODE` | empty | One-time setup code entered at `/setup` together with an owner email. Empty disables the private first-administrator flow. |
...
## Create a private first administrator

To evaluate the private-deployment onboarding instead of public signup, disable signup and set the owner email and a one-time setup code before the first start:
...
Then open [http://localhost:3005/setup](http://localhost:3005/setup), enter the owner email and the setup code from `.env`, and choose the administrator's name and password. The [first-administrator guide](/self-host/deploy-to-your-cloud/first-administrator) describes the flow and the boundary it enforces; only the way the values reach Den API differs between Compose and Helm.

Setup is available only while the database has no user, so set these variables before the first `up`. Once the administrator exists, `/setup` reports that setup is complete and the code cannot create another account. To start over, run `docker compose -f docker-compose.eval.yml down -v` and boot again.
