...
For Gateway opt-in, use the chart implementation introduced in `0.2.0` or later with matching Den API, Den Web, and Gateway images from the capability-version-1 release. The checkout's placeholder `appVersion` is not a compatible image recommendation. Follow [Enable OpenWork Gateway](/self-host/gateway) for explicit configuration and [Upgrade Gateway safely](/self-host/gateway-upgrade) for the quiesced database cutover before any existing installation rolls out these images. Legacy `inference.enabled: true` alone does not opt in, and OpenWork Models billing remains separate.
