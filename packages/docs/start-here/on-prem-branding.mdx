...
This maps `config.public.webOrigin` to `DEN_BASE_URL`; Den derives the Better Auth URL, CORS/trusted origins, API defaults, MCP resource defaults, and web-app hosts from it. Do not set it to `localhost`, a Kubernetes service name, or an internet hostname that member desktops cannot reach.

If you publish a separate Den API origin, set `config.public.apiOrigin` as the `DEN_API_PUBLIC_URL` compatibility override. Member devices must also resolve and trust that API origin for the one-time install-link exchange, external MCP clients, and signed branding asset URLs. See [Private network deployment](/start-here/private-network-deployment) for the single-origin nuance.
