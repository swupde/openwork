...
The sidebar's global **Archived** section lists the most recently archived sessions first across all workspaces, using the archive timestamp rather than creation or last-update time. Equal archive timestamps are ordered by workspace ID, then session ID (ascending), so rearranging workspaces or refreshing the session list does not reshuffle ties. Restoring a session removes it from Archived; archiving it again places it according to its new archive timestamp. Active sessions keep their existing pin, manual-order, and group behavior.
