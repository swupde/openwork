...
- Mirror all enabled images into an internal registry, including `openwork-den-api`, `openwork-den-web`, and `openwork-inference` if OpenWork Gateway or the retained legacy inference component is enabled. Gateway keeps this image name; do not substitute the separate `den-gateway` Web proxy.
...
For Gateway, mirror matching images from the capability-version-1 release rather than older stable images with new chart values. Follow [Gateway opt-in](/self-host/gateway) and the [safe upgrade procedure](/self-host/gateway-upgrade), including schema migration before runtime rollout.
