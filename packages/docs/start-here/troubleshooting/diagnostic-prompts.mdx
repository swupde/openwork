...
7. Call execute_capability with {"name":"getCapabilitiesGoogleWorkspaceCalendarAgenda","query":{"period":"today"}} and report the exact error or result shape.
...
12. execute_capability {"name":"getCapabilitiesGoogleWorkspaceCalendarAgenda","query":{"period":"today"}} → PASS = real events, or a clean needs_connection / missing-permission message — NOT unknown_capability. This capability resolves the current time and the member's primary-calendar timezone on the server; do not generate `timeMin`/`timeMax` for a relative agenda request.
