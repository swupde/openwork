...
It sets `NODE_EXTRA_CA_CERTS` to that file for `den-api`, `den-web`, enabled OpenWork Gateway (the retained `inference` workload), and the migration Job. CA rotation requires restarting the running workloads so Node reloads the file. Gateway's internal and desktop-reachable origins are separate; see [Gateway configuration](/self-host/gateway#internal-and-public-origins) for production HTTPS requirements.
