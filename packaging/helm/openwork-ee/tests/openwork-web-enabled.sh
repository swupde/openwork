...
assert_contains "$default_rendered" 'DEN_OPENWORK_WEB_ENABLED: "false"'

enabled_values="$tmp_dir/enabled-values.yaml"
enabled_rendered="$tmp_dir/enabled.yaml"
printf '%s\n' 'config:' '  public:' '    openworkWebEnabled: "true"' > "$enabled_values"
helm template openwork-ee "$chart_dir" -f "$enabled_values" > "$enabled_rendered"
assert_contains "$enabled_rendered" 'DEN_OPENWORK_WEB_ENABLED: "true"'

printf 'openwork-web-enabled chart checks passed\n'
