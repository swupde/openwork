...
export const OPENWORK_CLOUD_CONNECTION_INSTRUCTION =
  "The OpenWork Cloud connection is verified ready for this exact workspace/model. For primary Gmail inbox messages, call openwork-cloud_execute_capability directly with name native:google-workspace:getCapabilitiesGoogleWorkspaceGmailMessages and query { q: \"in:inbox\", maxResults? }. For primary Drive file search, call it with name native:google-workspace:getCapabilitiesGoogleWorkspaceDriveFiles and query { query, maxResults? }. For the primary Google Calendar agenda for today, tomorrow, or one local date, call it with name native:google-workspace:getCapabilitiesGoogleWorkspaceCalendarAgenda and query { day, maxResults? }. Omit timeZone so the connected account's calendar timezone is used. Do not search capabilities for these three bounded reads and do not load a Google Workspace skill first. For another org-connected service, use openwork-cloud_search_capabilities to search once with one precise query, then use openwork-cloud_execute_capability with an exact returned capability. Reuse an exact capability already returned in this task instead of searching again. Search a second time only when the first search returned no usable match or the server reports unknown_capability. Follow every returned parameter limit exactly, use default result limits first, batch independent reads in one tool round, fetch details only for selected records, and never repeat an unchanged failed call. Only mention services that search (or available_skills) actually returns. When a search result has kind mcp_app, execute that exact capability normally so OpenWork can render its originating standard MCP App; do not import it, ask for a standalone HTML URL, or require a generated direct-tool name. When a remote skill is listed under available_skills, call openwork-cloud_execute_capability with its <capability> directly; do not treat the local OpenCode skill list as the full inventory. Load each remote skill at most once per task. Reuse the full SKILL.md body already present in this task. Execute the skill capability again only if the provider returned unknown_capability or the user explicitly asks to reload changed instructions. Local OpenWork extensions remain available through openwork_query/openwork_execute with extension.actions and extension.call. Settings > Extensions is the member inventory surface for org and local apps. A successful search proves OpenWork Cloud itself is authorized, so a downstream connector failure does not mean OpenWork Cloud needs to be reconnected. If a result has kind connection_status, execute that exact match once: it returns the live status and renders an actionable connection card. Then name connectionStatus.connectionName and relay connectionStatus.action exactly: use Your Connections for the member, the organization Connections dashboard for an org admin, or the provider admin console for a provider-side failure. After the requested human fixes that connector, search again in the same task because results are live, not cached, so unchanged retries return the same error.";
